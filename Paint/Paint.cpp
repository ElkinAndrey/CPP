...
int WindowWidth = 750; // Ширина окна приложения
int WindowHeight = 500; // Высота окна приложения
int CenterX = WindowWidth / 2; // Центр координат по X
int CenterY = WindowHeight / 2; // Центр координат по Y
int Red = 0;
int Blue = 0;
int Green = 255;
int mouseX;
...
void Point(RGBQUAD* arr, int x, int y) {
    for (int i = y - 1; i <= y + 1; i++)
        for (int j = x - 1; j <= x + 1; j++) {
            if ((int(j) + WindowWidth * int(i) > 1) and (int(j) + WindowWidth * int(i) < WindowHeight * WindowWidth) and (int(j) >= 0) and (int(j) < WindowWidth)) {
                arr[(i)*WindowWidth + j].rgbRed = Red;
                arr[(i)*WindowWidth + j].rgbBlue = Blue;
                arr[(i)*WindowWidth + j].rgbGreen = Green;
            }
        }
}

void Line(RGBQUAD* arr, int x1, int y1, int x2, int y2) {
    int x, y;
    // Поменять местами точки, если линия идет назад 
    float k = float((y2 - y1)) / float((x2 - x1));
    if ((abs(k) >= 1 and x2 - x1 > 0 and y2 - y1 < 0) or (y2 - y1 <= 0 and x2 - x1 <= 0) or (abs(k) < 1 and x2 - x1 < 0 and y2 - y1 > 0)) {
...
void func(HDC hdc) {
    ...
    if (click) {
        Point(arr, mouseX, mouseY);
        Line(arr, mouseX, mouseY, firstClickX, firstClickY);
        Line(arr, mouseX, mouseY, lastClickX, lastClickY);
        if (!firstClick) {
            lastClickX = mouseX;
            lastClickY = mouseY;
        }
